clear all;

array1_43 = csvread('/home/ahmasri/ns-allinone-3.26/ns-3.26/Official_collection_Of_Results/MLB/Run_13p/DlRsrpSinrStats.txt');
array2_44 = csvread('/home/ahmasri/ns-allinone-3.26/ns-3.26/Official_collection_Of_Results/MLB/Run_26p/DlRsrpSinrStats.txt');
array3_45 = csvread('/home/ahmasri/ns-allinone-3.26/ns-3.26/Official_collection_Of_Results/MLB/Run_27p/DlRsrpSinrStats.txt');
array4_46 = csvread('/home/ahmasri/ns-allinone-3.26/ns-3.26/Official_collection_Of_Results/MLB/Run_16p/DlRsrpSinrStats.txt');



 NumEnb = 9;
 cellIds = 1:1:NumEnb;

 %Sort per cell
 Sorted1 = sortrows(array1_43, 2);
 Sorted2 = sortrows(array2_44, 2);
 Sorted3 = sortrows(array3_45, 2);
 Sorted4 = sortrows(array4_46, 2);
 
 %Hist1 = zeros(8 , NumEnb);
 %Hist2 = zeros(8 , NumEnb);
 %Hist3 = zeros(8 , NumEnb);
 %Hist4 = zeros(8 , NumEnb);
 
 % we will just look on one cell for different Tx powers
 %for i = 1:NumEnb  
 i = 8;
    List1 = Sorted1(Sorted1(:,2) == i, 1:6);
    Hist1 = getCount(List1); % return list (8 x 1)
    
    List2 = Sorted2(Sorted2(:,2) == i, 1:6);
    Hist2 = getCount(List2); % return list (8 x 1)
    
    List3 = Sorted3(Sorted3(:,2) == i, 1:6);
    Hist3 = getCount(List3); % return list (8 x 1)
    
    List4 = Sorted4(Sorted4(:,2) == i, 1:6);
    Hist4 = getCount(List4); % return list (8 x 1)
    
 %end

 tmp1 = sum(Hist1);
 total1 = sum(tmp1);
 
 tmp2 = sum(Hist2);
 total2 = sum(tmp2);
 
 tmp3 = sum(Hist3);
 total3 = sum(tmp3);
 
 tmp4 = sum(Hist4);
 total4 = sum(tmp4);
 
 TotalHist = [Hist1/total1 Hist2/total2 Hist3/total3 Hist4/total4];
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Figure One%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 f1 = figure('Name', 'SINR/Cell Vs. TxPower', 'NumberTitle', 'off');           % Figure  
 
 bar(TotalHist)
 hold on;
 
 set (gca, 'xticklabel', { '< 0 dB', '0-5 dB', '6-10 dB', '11-15 dB', '16-20 dB', '21-25 dB','26-30 dB', '>30 dB'})
  % Set the axis limits
 %axis([0 9 0 0.12])
 %set(gca,'YTick',0:0.01:0.12);
 xlabel('SINR [dB]')
 ylabel('CDF')
 legend ({'43.0 dBm', '44.0 dBm','45.0 dBm', '46.0 dBm'} ,'location', 'best');
 %legend (h, 'location', 'northeastoutside');
 title ('Using N = 150 Users, Speed = 5 m/sec, Distance = 750 m, Cell 5');
 %set (h2, 'fontsize', 12);
 saveas(f1,'Plots/DlRsrpSinrStats_AvgSINRvsDistance43_44_45_46dBm.png');  % Save plot
 savefig('Plots/DlRsrpSinrStats_AvgSINRvsDistance43_44_45_46dBm.fig');

 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Figure Two%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Figure One%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 
 f1 = figure('Name', 'SINR/Cell Vs. TxPower', 'NumberTitle', 'off');           % Figure  
 
 plot(TotalHist)
 hold on;

 set (gca, 'xticklabel', { '< 0 dB', '0-5 dB', '6-10 dB', '11-15 dB', '16-20 dB', '21-25 dB','26-30 dB', '>30 dB'})
  % Set the axis limits
 %axis([0 9 0 0.12])
 %set(gca,'YTick',0:0.01:0.12);
 xlabel('SINR [dB]')
 ylabel('CDF')
 legend ({'43.0 dBm', '44.0 dBm','45.0 dBm', '46.0 dBm'} ,'location', 'best');
 %legend (h, 'location', 'northeastoutside');
 title ('Using N = 150 Users, Speed = 5 m/sec, Distance = 750 m, Cell 5');
 %set (h2, 'fontsize', 12);
 saveas(f1,'Plots/DlRsrpSinrStats_AvgSINRvsDistance43_44_45_46dBm_linear.png');  % Save plot
 savefig('Plots/DlRsrpSinrStats_AvgSINRvsDistance43_44_45_46dBm_linear.fig');

 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Figure Two%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
